...
Финальное решение появилось не из одного training run. Основной цикл разработки выглядел так: фиксировали рабочий baseline, меняли один компонент, отдельно проверяли поведение двух категорий, разбирали ошибки, после чего либо оставляли изменение, либо откатывали его. Для нас было важно не только получить более сильную модель, но и понять, за счет какого именно компонента меняется качество.
